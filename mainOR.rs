...
fn main() {
	...
	let encoder = Encoder::new(0., 255., 1, 1).expect("Can't instantiate encoder");
	...
	write_image(img_buff_1, String::from("mickey_fhe.png")).expect("Can't write image");
	//write_image(img_buff_2, String::from("mickey_inv_fhe.png")).expect("Can't write image");

}

fn mul_ciphertext(c1: &mut VectorLWE, c2: &VectorLWE, bsk: &LWEBSK) -> Result<VectorLWE, CryptoAPIError> {
	let result = c1.mul_from_bootstrap(c2, bsk)?;
	Ok(result)
}
